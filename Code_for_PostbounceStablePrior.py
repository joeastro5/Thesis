...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 23 00:00:45 2021
...
lneps = np.linspace(2,9,dim)#(log(7),log(5000),dim)
n = np.linspace(3,7,dim)
logV = np.linspace(-55,-45,dim)
q = np.linspace(0.2,1.25,dim)
            ...
            for l in range(start):  #n
               ...
               # c_ek = sqrt(2*eps_ek)
                ...
                if  log10_10As_th >= Asmax or log10_10As_th <= Asmin or abs(fNL) >=1e2 or abs(gNL) >= 1e6 or abs(alpha_th)>0.1:
                    ...
                    numinside += 1
                   # if i == start-1 or i == 0 or k == start-1 or k==0 or l ==start-1 or l==0 or j==0 or j==start-1:
                ...
                if any(abs(y-log10_10As) < As_thresh for y in As_prev[int(i/2)][int(l/2)][int(k/2)]):
                    ...
                    for m in range(int(2**(1+z)*start)):#n
                      ...
                      #  c_ek = sqrt(2*eps_ek)
                        ...
                        alphas_th = ekphase(nr[m],bhere)[1]
