...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 29 01:23:31 2021
...
from Integ_deltas import new_result
...
def ekphase(n,b,N = 60):
    eps_ek = 3*(N+1)**n#exp(lneps)*(N+1)**n
    epsN = n*eps_ek/(N+1)
...
lneps = np.linspace(2,8,dim)#(log(7),log(100),dim)
n = np.linspace(0,2,dim)
logV = np.linspace(-51,-47,dim)
    ...
    for j in range(start): #q
        ...
        for k in range(start): #logV
            ...
            for l in range(start):  #n
                ...
                #c_ek = sqrt(2*eps_ek)
                alpha_th = ekphase(nhere[l],b)[1] 
                ns_th = ns
                log10_10As_th = log(1e10*exp(logAs+ log((tau*kval/2)**(ns-1))))#log((tau*kval/2)**(-2*c_ek*(b-c_ek)/(c_ek**2-2)))))
            ...
            for k in range(int(2**z*start)):#lnV
                if any(y>threshold for y in like_prev[i][l][k]):
                    ...
                    for m in range(int(2**z*start)):#n
                        ...
                        alphas_th1 = ekphase(nr[2*m+1],get_b(nr[2*m+1]))[1]
                        alphas_th2 = ekphase(nr[2*m],get_b(nr[2*m]))[1]
                        ...
                        #this = np.dot(inverse_primordial_covariance,yd_minus_yth)
                        #chisq =  np.dot(np.transpose(yd_minus_yth),this)  # is it < dof?   
                        #if np.any(np.isinf(yd_minus_yth)):
                        #    chisq = inf
                        #likelihoodr[i][l][k][m]= exp(-.5*chisq)
                        #summation += exp(-.5*chisq)
                        #L_lnL += exp(-.5*chisq)*(-.5*chisq)
                        #numpoints += 1
